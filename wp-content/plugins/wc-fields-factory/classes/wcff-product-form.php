...
class wccpf_product_form {	
						...
						if( isset( $_REQUEST[ $field["name"] ] ) || isset( $_FILES[ $field["name"] ] ) ) {							
							if( $field["type"] != "checkbox" && $field["type"] != "file" ) {		
								...
								if( isset( $field["upload_url"] ) ){									
									if( $field["upload_url"] != "" ){	
										Global $copy_field_upload_dir;
										$copy_field_upload_dir = $field["upload_url"];
				...
				if( count( $admin_fields ) > 0 ) {						
					foreach ( $admin_fields as $title => $afields ) {
						if( count( $afields ) > 0 ) {
							foreach ( $afields as $key => $afield ) {
								$afield["show_on_product_page"] = isset( $afield["show_on_product_page"] ) ? $afield["show_on_product_page"] : "no";
								$admin_multiple_check = isset( $afield["single_or_multi"] ) ? $afield["single_or_multi"] : array();
	...
	function custom_upload_dir( $urls ){
		Global $copy_field_upload_dir;
		$urls['path'] = WP_CONTENT_DIR .'/'. $copy_field_upload_dir;
		$urls['url']  = WP_CONTENT_URL .'/'. $copy_field_upload_dir;
		return $urls;
	}
	...
	function save_wccpf_order_meta( $item_id, $values, $cart_item_key ) {	
		$values = version_compare( WC()->version, '3.0.6', '<' ) ? $values : isset( $values->legacy_values ) ?  $values->legacy_values : $values;	
